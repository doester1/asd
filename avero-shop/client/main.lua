...
RegisterNUICallback(
    "exchange",
    function(data)
        TriggerServerEvent("core_credits:exchange", data["item"])
    end
)
...
RegisterNUICallback(
    "redeem",
    function(data)
        TriggerServerEvent("core_credits:redeem", data["item"])
    end
)
    ...
    SendNUIMessage(
        {
            ...
            header = string.upper('Zakup Kredytow'),
            description = 'Kredyty możesz obecnie zakupić na naszym Discordzie który jest podpięty w buttonie poniżej',
