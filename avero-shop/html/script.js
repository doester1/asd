...
function closeMenu() {
  $.post('https://avero-shop/close', JSON.stringify({}));
...
function buyItem(item) {
  ...
  $.post('https://avero-shop/buyItem', JSON.stringify({
    item: item
  }));
}
...
function openAreYouSure(item) {
  ...
  var base = '<div id="sure_gradient" class="slide-right">' +
    ...
    '   </div>' +
    '<div id="sure_confirm" onclick="buyItem(\'' + item + '\')" class="ripple">Zakup</div>' +
    '<div id="sure_cancel" onclick="cancelAreYouSure()" class="ripple">Anuluj</div>' +
    '<div>';
...
function redeem(item) {
  ...
  $.post('https://avero-shop/redeem', JSON.stringify({
    item: item
  }));
...
function exchange(item) {
  ...
  $.post('https://avero-shop/exchange', JSON.stringify({
    item: item
  }));
...
function winningChoose(item, t) {
  ...
  var base = '<div id="sure_gradient" class="slide-right">' +
    ...
    '    <p>OPCJE</p>' +
    '   </div>' +
    '<div id="sure_redeem" onclick="redeem(\'' + item + '\')" class="ripple">Odbierz</div>' +
    '<div id="sure_exchange" onclick="exchange(\'' + item + '\')" class="ripple">Wymień</div>' +
    '<div id="sure_close" onclick="cancelAreYouSure()" class="ripple">Zamknij</div>' +
    '<div>';
...
function openTasks() {
  ...
  playClickSound();
    ...
    $.post('https://avero-shop/openedTasks', JSON.stringify({}));
...
function openCaseOpening() {
  ...
  var base = '<div class="clearfix borderbox slide-right" id="creditmenu"><!-- column -->' +
    ...
    '<div id="category" class="ripple" onclick="openCaseOpening()">Otwórz Skrzynkę</div>' +
    '<div id="category" class="ripple" onclick="openWinnings()">Odbierz Nagrody</div>' +
  ...
  base = base + '    </ul>' +
    ...
    '  <div class="button5 ripple" onclick="clearValue()">Wyczyść</div>' +
    '</div>' +
  ...
  startSpinnerBtn.addEventListener("click", (e) => {
    ...
    if(betAmount < lowestExchange) {
       ...
       $.post('https://avero-shop/sendMessage', JSON.stringify({
      message: 'bet_limit'
    }));
      return;
    }
    ...
    od.update(credits)
    $.post('https://avero-shop/removeCredits', JSON.stringify({
...
function itemWon(win) {
  ...
  $.post('https://avero-shop/addWinning', JSON.stringify({
    item: win
  }));
...
function openWeb() {
  ...
  window.invokeNative('openUrl', 'https://' + 'discord.gg/avero-gg');
...
function openBuy() {
  ...
  var base = '<div class="clearfix borderbox slide-right" id="creditmenu"><!-- column -->' +
    ...
    '<div id="buy_button" onclick="openWeb()" class="ripple">ZAKUP</div>' +
